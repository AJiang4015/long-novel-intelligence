...
Run: `cd backend && python -m pytest tests/unit/test_hygiene.py -k "rc3" -v 2>&1 | Select-Object -Last 16`

Expected: `test_rc3_brother_no_candidate_not_registered` FAIL（当前 兄弟 classify=None → PERSON 注册）；`test_rc3_father_mother_grandfather_not_filtered` PASS（PERSON 现状）；`test_rc3_generic_overrides_llm_person_category` FAIL（当前 LLM PERSON 直接注册，precedence 未生效）；红状态确认 gap。

- [ ] **Step 3: Commit（红）**

```bash
git add backend/tests/unit/test_hygiene.py
git commit -m "test: V0.2.4-b RC3 relational generic 词表测试（12 项，含 precedence 锁死）"
```
...
- [ ] **Step 2: resolver `_resolve_name` 桥接（明确 category precedence）**

在 `_resolve_name` 中，category 决策采用**明确优先级**（relational-generic exact-match 高于 LLM category）：

```python
        candidates = self._recall(name, confirmed, text_confirmed)
        # V0.2.4-b RC3：category precedence（评审锁定 2026-08-23）——
        # 1. COLLECTIVE / INVALID hard rules（已在 is_hard_filtered 分支处理）
...
**1. Spec 覆盖（对照设计 + 3 约束 + precedence 修改）：**
...
- **precedence（评审新增）：relational-generic exact-match 强制 GENERIC，覆盖 LLM PERSON** → Task 2 Step 2 `hy_cat == GENERIC → cat = GENERIC`（先查 hygiene 再查 LLM）+ test_rc3_generic_overrides_llm_person_category / test_rc3_generic_overrides_llm_person_with_candidate_judge ✓
- **注释要求：祖父/父亲/母亲不进词表是《边城》当前数据验证的项目级决策，非通用语义规则** → Task 2 Step 1 词表注释需明确 ✓
