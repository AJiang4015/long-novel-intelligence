...
**重复边界**：跨文档允许**简短引用 / 摘要 / 指针**；只有「完整规则」才要求唯一事实源——**不要追求零重复**。例：P16-b 可同时出现在本文件（指针 → D-6）、`PIPELINE_LAYER.md`（归属细节）、`DECISIONS.md`（完整决策）、`PROBLEM.md`（问题历史），但完整规则只在其事实源各写一份，其余位置只放指针。
...
业务行为（代码 / 测试 / prompt / 配置 / 数据语义）发生变化时，先检查该行为对应的事实源文档：`DECISIONS.md`（决策）、`PROBLEM.md`（问题历史）、`TESTING.md`（验证规范）、`*_LAYER.md`（层边界与归属）。若已有文档描述旧行为，**必须在同一变更中同步更新对应事实源**，禁止「代码已到 V0.2.9、文档仍写 V0.2.6 行为」的脱节状态。检查流程见 `PROCESS.md` §4.5。

新行为若无对应文档（且不构成长期决策/问题），不强制为此新增文档——但一旦纳入决策或问题体系，即受本纪律约束。
