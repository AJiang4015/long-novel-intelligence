...
- **当前状态**: ✅ **implemented + 首份真实基线已产出（2026-08-28），P20 收尾完成；checkset v2 基线已提交（INVALID，如实保留）**。框架：`backend/tools/eval_framework/`（checkset 声明式检查集 24 条（v2） + runner/evidence/baseline/report，纯函数判定；PASS/FAIL/OBSERVATION/INCONCLUSIVE/SKIP；`--dry-run/--smoke/--runs` 已接线，baseline/compare CLI 未接——按 Spec §15 保持现状）。**基线状态**：v1（`biancheng-2026-08-28-deepseek-v4-flash-0731.json`）与 v2（`…-checkset2.json`）均为 `INVALID_NOT_REGRESSION_SAFE`——v1 因 A1 稳定失败（老二 漏提，D-19 收敛后移除）；**v2 因 C3 稳定失败（爹爹 ∈ 顺顺.aliases FAIL×3）**——C3 保留 hard gate 不降级（爹爹 5 次出现非单次低显著性，D-19 不适用；EXTRACTION_LAYER 归因已并入 P017 D5-a 实例）。P16-b 系（C1/C2/C4）与 G1/G3/G5 稳定 PASS；F1 因 merge_judge 6MB 限制恒 INCONCLUSIVE/观察（沿用 P19 跟进项）。v2 基线 artifact `docs/evaluation/baselines/biancheng-2026-08-28-deepseek-v4-flash-0731-checkset2.json` + [v2 基线报告](docs/evaluation/2026-08-28-biancheng-quality-baseline-checkset2.md)。约束全程成立：P19 checkpoint 语义零改动（eval 强制 `er_checkpoint_enabled=False`）、不重开 P16/P17/P18、不进入性能优化、不修改 expectation 适配结果（v2 的 A1 修订是显式验收边界决策 D-19；C3 未降级）
